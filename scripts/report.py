...
def collect_sensor_data(client: WAB11SyncClient) -> dict[str, Any]:
    ...
    data = {
        ...
        "hot_water": {
            ...
            "status": client.hot_water.status.name
            if hasattr(client.hot_water.status, "name")
            else str(client.hot_water.status),
        },
        "heat_pump": {
            "is_configured": client.heat_pump.is_configured,
            "operating_state": client.heat_pump.operating_state.name
            if hasattr(client.heat_pump.operating_state, "name")
            else str(client.heat_pump.operating_state),
    ...
    for i, hk in enumerate(client.heating_circuits, 1):
        ...
        if hk.is_configured:
            hk_data.update(
                {
                    "config": hk.config.name
                    if hasattr(hk.config, "name")
                    else str(hk.config),
...
def flatten_data_for_csv(data: dict[str, Any]) -> dict[str, Any]:
    ...
    flat = {
        ...
        "sh_bivalence_temp_hot_water": data["secondary_heat"][
            "bivalence_temp_hot_water"
        ],
        # Inputs
    ...
    for i in range(1, 6):
        ...
        if hk_data and hk_data.get("is_configured"):
            ...
            flat[f"{prefix}room_setpoint_effective"] = hk_data.get(
...
def generate_text_report(data: dict[str, Any]) -> str:
    ...
    if hp_data["is_configured"]:
        ...
        lines.append(
            f"  Suction Gas Temp:   {format_temp(hp_data['suction_gas_temp'])}"
        )
        ...
        lines.append(
            f"  Supports Cooling:   {format_bool(hp_data['supports_cooling'])}"
        )
    ...
    if configured_circuits:
        ...
        for hk in configured_circuits:
            status = (
                "🔥" if hk.get("is_heating") else ("❄️" if hk.get("is_cooling") else "•")
            )
            lines.append(f"  HK{hk['id']}: {hk['config']}")
            lines.append(f"    Mode:             {hk['mode']}")
            lines.append(
                f"    Room Temp:        {format_temp(hk['room_temp'])}"
                f" → {format_temp(hk['room_setpoint_effective'])} {status}"
            )
            if hk.get("room_humidity") is not None:
                lines.append(f"    Room Humidity:    {hk['room_humidity']}%")
            lines.append(
                f"    Flow Temp:        {format_temp(hk['flow_temp'])} → {format_temp(hk['flow_setpoint'])}"
            )
            lines.append(
                f"    Setpoints:        Comfort: {format_temp(hk['setpoint_comfort'])}, "
            ...
            if party_mode != "automatic" and party_hours:
                lines.append(
                    f"    Party/Pause:      {party_mode.upper()} ({party_hours}h)"
                )
            lines.append("")
    ...
    if hw_data["is_configured"]:
        ...
        lines.append(
            f"  Temperature:        {format_temp(hw_data['temperature'])}"
            f" → {format_temp(hw_data['setpoint_effective'])}"
        )
        lines.append(f"  Normal Setpoint:    {format_temp(hw_data['setpoint_normal'])}")
        lines.append(
            f"  Setback Setpoint:   {format_temp(hw_data['setpoint_setback'])}"
        )
        if hw_data["is_push_active"]:
            lines.append(
                f"  Push Active:        ✅ {hw_data['push_minutes']} min remaining"
            )
        lines.append("")
    ...
    if (
        sh_data["is_wez2_configured"]
        or sh_data["is_e1_configured"]
        or sh_data["is_e2_configured"]
    ):
        lines.append("─" * 70)
        ...
        lines.append(
            f"  Bivalence Temp:     {format_temp(sh_data['bivalence_temp_heating'])}"
        )
    ...
    if inp_data["is_evu_lock"]:
        lines.append("  🔒 EVU Lock Active")
    if inp_data["is_sg_maximum"]:
        lines.append("  ⚡ SG Maximum Active (PV Surplus)")
...
def run_single_collection(client: WAB11SyncClient, args) -> dict[str, Any]:
    ...
    if args.csv:
        ...
        if appended:
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Data appended to {args.csv}",
                file=sys.stderr,
            )
        else:
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CSV file created: {args.csv}",
                file=sys.stderr,
            )
    ...
    if args.output:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Report written to {args.output}",
            file=sys.stderr,
        )
...
Examples:
    ...
    parser.add_argument(
        "--host", required=True, help="IP address of the WAB11 controller"
    )
    parser.add_argument(
        "--port", type=int, default=502, help="Modbus TCP port (default: 502)"
    )
    parser.add_argument(
        "--json", action="store_true", help="Output in JSON format instead of text"
    )
    parser.add_argument(
        "--csv",
        type=str,
...
def main():
    ...
    if args.interval and not args.csv:
        print(
            "Warning: --interval without --csv will only print to stderr",
            file=sys.stderr,
        )
...
def run_continuous(args):
    ...
    print(
        f"Starting continuous monitoring of WAB11 at {args.host}:{args.port}",
        file=sys.stderr,
    )
    print(
        f"Interval: {args.interval} minutes ({interval_seconds:.0f} seconds)",
        file=sys.stderr,
    )
